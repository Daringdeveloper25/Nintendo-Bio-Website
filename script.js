...
// Select all elements with class "tab-content"
// These are the sections that will be shown or hidden
var contents = document.querySelectorAll(".tab-content");


// Loop through every button so we can attach a click event to each one
buttons.forEach(function (button) {

  // Add a click event listener to each button
  button.addEventListener("click", function () {

    // Get the value of the button's data-tab attribute
    // Example: data-tab="home" → tab = "home"
    var tab = button.dataset.tab;

    // Hide ALL tab content sections first
    // We remove the "active" class from every content block
    // If this function is gone, all three pictures will show when the button is clicked.
    contents.forEach(function (content) {
      content.classList.remove("active");
    });

    // Remove "active" class from ALL buttons
    // This resets button styling so none look selected
    // If this function is gone, it makes all buttons appear active when clicked on.
    buttons.forEach(function (btn) {
      btn.classList.remove("active");
    });

    // Show the correct tab content section
    // Finds the element with matching id (e.g. id="home")
    document.getElementById(tab).classList.add("active");

    // Highlight the clicked button
    // Adds "active" class so it looks selected
...
/*Divide Between Tab Buttons and Mario Image*/




let marioImg = document.getElementById("marioPic");
